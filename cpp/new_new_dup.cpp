...
int i=0,j,k,kop[10],kop1,fs=0,cnt=0,i1,j2,head=0,l=0,countdic=0,dicwrdindex=0,mainie=0,pof=0,p=0,pop=0,vm=1,m=1;
...
//ROW 1 (SENTENCE PRINT)
	...
	{
		//cout<<w[i][0]<<" ";	
		outFile << w[i][0]<<" ";		//GIVEN SENTENCE PRINTED
	}
	//cout<<endl;
	outFile <<"\n";
...
//MAIN PART
	...
	char ltr;
	ifstream infile;
	infile.open("/home/vaishnavi/language_communicator/dictionaries/H_concept-to-mrs-rels.dat"); //dictionary open
	...
	//while (infile)
	{
			...
			for(i=0;i<cnt;i++)
			{
				...
				else if((w[i][2]=="VM") && (w[i+1][6]=="lwg__vaux"))
				{
					//cout<<w[i][0]<<" ";
			...
			p=0;
        		...
        		for(i=0;i<(cnt-1);i++)
        		{
       			...
       			if((w[i][2]=="VM"))
        			...
        			{	
        				if((w[i][2]=="VM") || (w[i][6]=="lwg__vaux") || (w[i][6]=="lwg__vaux_cont")) 
        				{
							...
							for(j=0;j<lastwrd[pop];j++)
							{
								for(k=j;k<lastwrd[pop];k++)
								{
									v+=verb[pop][k];
									//cout<<verb[k]<<" ";
									//break;
							...
							int how=0;
								//cout<<i<<(i+1);
								//cout<<"h";
							if((vaux[pop]=="hE") || (vaux[pop]=="hEM") || (vaux[pop]=="ho"))
							{
								//cout<<"state_copula-pres";
								outFile << w[i][6];	//"state_copula-pres";
								p++;
							}
							else if((vaux[pop]=="WA") || (vaux[pop]=="WI") || (vaux[pop]=="We") || (vaux[pop]=="WeM") || (vaux[pop]=="WIM"))
							{
								//cout<<"state_copula-past";
								outFile << w[i][6];	//"state_copula-past";
								p++;
							}
							else
							{
	...
	for(i=0;i<grp;i++)
	{
		...
		else if((w[i][6]=="nmod__adj") || (w[i][6]=="adv") || (w[i][6]=="pof__cn") || (w[i][6]=="jjmod__intf") || (w[i][6]=="r6"))
		{
			...
			else if(w[i][6]=="pof__cn")
			{
				...
				outFile <<mainie<<"."<<(i+1)<<"/"<<mainie<<"."<<j<<":"<<"pof__cn"<<" ";
			}
			else
			{
				//cout<<mainie<<":"<<w[i][6]<<" ";
				outFile <<mainie<<":"<<w[i][6]<<" ";
			}
		}
		else if((b==kop[k] && w[i][6]!="pof" && w[i][6]!="rsym" && w[i][6]!="lwg__vaux" && w[i][6]!="lwg__vaux_cont" && w[i][2]!="VM" && w[i][6]!="lwg__psp"))
		{
			k++;
			//cout<<kop[k]<<" ";
			for(j=1;j<dicwrdindex;j++)
	...
	for(j=0;j<unavl;j++)
        {
        	if((w[j][6]!="lwg__psp") && (w[j][6]!="rsym") && (w[j][2]!="NNP") && (w[j][2]!="NNPC") && (w[j][2]!="PRP"))
        	{
				...
				for(i=0;i<unavl;i++)
				{
				 ...
				 outFile << "not found in dictionary.]\n";
				 //cout<<endl;
				 break;
			}
		}
